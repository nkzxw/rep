...
class WeiBoLogin(object):
    ...
    def login(self, user_name, pass_word):
        ...
        if json_data_1["retcode"] == "0":
            ...
            if json_data_2["result"] is True:
                ...
                logging.warning("WeiBoLogin succeed: %s-%s", self.user_uniqueid, self.user_nick)
            else:
    ...
    def comment(self, url, content):
        ...
        post_data = {
            ...
            "iframe": 1,
            "usertype": "pc",
            "callback":  "iJax_%d" % (int(time.time())),
        }
        comment_url = "http://comment5.news.sina.com.cn/cmnt/submit"
        self.session.headers.update({"Referer": url})
        response = self.session.post(comment_url, data=post_data)
        logging.warning("WeiBoLogin comment: %s", response.text)

if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s\t%(levelname)s\t%(message)s", filename='./logmsg.log')
    weibo = WeiBoLogin()
    weibo.login("17197471430", "KeYiYou123")
    weibo.comment("http://sports.sina.com.cn/basketball/nba/2018-08-20/doc-ihhxaafz1871821.shtml", "好好好好好好")